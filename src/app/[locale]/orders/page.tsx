...
export default function OrdersPage() {
  ...
  const updateOrderStatus = (orderId: string, newStatus: Order['status']) => {
    ...
    if (newStatus === 'completed') {
      ...
      if (orderToComplete) {
        setRatingModal({
          ...
          orderToComplete: orderToComplete
        });
        return; // Don't update status yet, wait for rating
      }
    }
    ...
    localStorage.setItem('specialistOrders', JSON.stringify(updatedOrders));
  };
  ...
  const handleRatingSubmit = async (rating: number, comment: string) => {
    try {
      ...
      const ratingData = {
        ...
        raterType: 'specialist', // The specialist is rating the client
        targetType: 'client',
        targetUserId: ratingModal.orderToComplete?.clientPhone, // Using phone as identifier for now
      ...
      console.log('Rating data to be sent to API:', ratingData);
      
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));

      // Save rating locally for now (until API is ready)
      ...
      if (ratingModal.orderToComplete) {
        const updatedOrders = orders.map(order =>
          order.id === ratingModal.orderId ? { ...order, status: 'completed' as Order['status'] } : order
        );
        setOrders(updatedOrders);
        localStorage.setItem('specialistOrders', JSON.stringify(updatedOrders));
      }
      ...
      toast.success('Buyurtma muvaffaqiyatli bajarildi va baho yuborildi!', {
        position: 'top-center',
        autoClose: 3000,
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-6">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('navigation.orders')}</h1>
          <p className="text-gray-600">Mijozlardan kelgan buyurtmalarni boshqaring</p>
        </div>
              ...
              >
                ...
                {pendingCount > 0 && (
                  <span className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${
                    activeTab === 'new' ? 'bg-white text-teal-600' : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {pendingCount}
                  </span>
                )}
          ...
          <>
            {newOrders.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Yangi buyurtmalar yo'q</h3>
                <p className="text-gray-500">Mijozlardan yangi buyurtmalar kelganda, ular shu yerda ko'rinadi</p>
              </div>
            ) : (
              <div className="space-y-6">
                {newOrders.map((order) => (
                  <div key={order.id} className="bg-white rounded-2xl shadow-lg p-6 border-l-4 border-yellow-400">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                      <div>
                        ...
                        <p className="text-gray-900 font-medium">{order.service}</p>
                      </div>
          ...
          <>
            {historyOrders.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Buyurtmalar tarixi bo'sh</h3>
                <p className="text-gray-500">Qabul qilgan, bajargan yoki rad etgan buyurtmalar shu yerda ko'rinadi</p>
              </div>
            ) : (
              <div className="space-y-6">
                {historyOrders.map((order) => (
                  <div key={order.id} className={`bg-white rounded-2xl shadow-lg p-6 border-l-4 ${
