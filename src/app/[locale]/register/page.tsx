...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isPhoneExists = allUsers.some((user) => user.phone === formData.phone);
    ...
    try {
      ...
      const sendOtp = await axios.post("https://ik-1.onrender.com/api/v1/verification/send", {
        type: "register", 
        phone: formData.phone
      });
      
      // Navigate to phone verification page
      router.push(`/verify-phone?phone=${encodeURIComponent(formData.phone)}&purpose=register`);
    } catch (error) {
      console.error("Error sending OTP:", error);
      if (axios.isAxiosError(error)) {
        const errorMessage = error.response?.data?.message || t('error_sending_otp');
        alert(errorMessage);
      } else {
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl p-6 sm:p-8 max-w-2xl w-full border border-white/20">
          <div className="text-center mb-6 sm:mb-8">
            <h1 className="text-3xl sm:text-4xl font-bold text-teal-900 mb-3">Fixoo</h1>
            <p className="text-gray-500 text-base sm:text-lg">
              {role === "MASTER" ? t('auth.specialist_description') : t('auth.client_description')}
            </p>
          </div>
            ...
            {role === "MASTER" && (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('select_profession')} <span className="text-red-500">*</span>
                    </label>
                    <select
                      name="profession"
                      ...
                      required
                    >
                      <option value="" disabled className="text-gray-400">{t('select_profession')}</option>
                      {professions.map((prof) => (
                        <option key={prof.value} value={prof.value} className="text-gray-900">
                          {prof.label}
                        </option>
                      ))}
                    </select>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('living_address')} <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                      name="add_address"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      required
                    >
                      <option value="" disabled className="text-gray-400">{t('select_region')}</option>
                      {regions.map((region) => (
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('select_district')} <span className="text-red-500">*</span>
                    </label>
                    <select
                      name="district"
                    ...
                    >
                      <option value="" disabled className="text-gray-400">
                        {!formData.region ? t('select_district_first') : t('select_district')}
                      </option>
